        ...
        with nogil:
            for i in xrange(imax):
                for j in xrange(jmax):
                    ...
                    self.s_flux[ij] = entropyflux_from_thetaflux_qtflux(SHF, LHF, RS.p0_half[gw],
                                                                        DV.values[temp_shift+ijk], PV.values[qt_shift+ijk], DV.values[qv_shift+ijk])
...
cdef class SurfaceGCMVarying(SurfaceBase):
    ...
    cpdef update(self, Grid.Grid Gr, ReferenceState.ReferenceState RS, PrognosticVariables.PrognosticVariables PV,
        ...
        if self.alt_gustiness:
            compute_windspeed(&Gr.dims, &PV.values[u_shift], &PV.values[v_shift], &windspeed[0], RS.u0, RS.v0, self.scaled_gustiness)
        else:
            compute_windspeed_gust(&Gr.dims, &PV.values[u_shift], &PV.values[v_shift], &windspeed[0], RS.u0, RS.v0, self.scaled_gustiness)
        ...
        cdef:
            ...
            double theta_rho_g = theta_rho_c(RS.Pg, self.T_surface, qv_star, qv_star)
            double s_star = sd_c(pd_star,self.T_surface) * (1.0 - qv_star) + sv_c(pv_star, self.T_surface) * qv_star
            double thli_star =  self.T_surface / exner_c(RS.Pg)
